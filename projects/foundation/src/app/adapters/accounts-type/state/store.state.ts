...
} from '../../../domain/accounts-type/models';
...
export const accountsTypeStore = signalStore(
  ...
  withMethods((store, repositoryService = inject(AccountsTypeRepositoryService), errorService = inject(ErrorService)) => {
    return {
      load: rxMethod<void>(
        pipe(
          debounceTime(300),
          ...
          switchMap(() => {
            return repositoryService.fetchAll().pipe(
              tapResponse({
                ...
                error: (error: ErrorType) => patchState(store, { error: errorService.handleError(error) }),
                finalize: () => {
        ...
        pipe(
          ...
          switchMap((params) => {
            return repositoryService.findOne(params.id).pipe(
              tapResponse({
                next: (response: AccountsTypeResponseDTO) => patchState(store, { selected: response, error: null, createdOrUpdated: null }),
                error: (error: ErrorType) => patchState(store, { error: errorService.handleError(error) }),
                finalize: () => {
                  patchState(store, { isLoading: false });
                },
          ...
          switchMap((params) => {
            return repositoryService.create(params.accountsType).pipe(
              tapResponse({
                ...
                error: (error: ErrorType) => patchState(store, { error: errorService.handleError(error), createdOrUpdated: null }),
                finalize: () => {
                  patchState(store, { isLoading: false });
                },
              }),
            ...
            return repositoryService.update(params.id, params.accountsType).pipe(
              tapResponse({
                next: (response: AccountsTypeResponseDTO) => {
                  ...
                  const index = sorted.findIndex((x: AccountsTypeResponseDTO) => x.id == response.id);
            ...
            return repositoryService.delete(params.id).pipe(
              tapResponse({
                next: () => {
                  ...
                  const newSorted = items.filter((x: AccountsTypeResponseDTO) => x.id !== params.id.value!);
      ...
      toggleDialogHadBeenOpened: (
        opened: boolean,
        title: string,
        action: string,
        submitButtonText?: string,
        cancelButtonText: string = BUTTON_LABELS.dialog.cancel,
      ) => {
        patchState(store, {
          dialog: { opened: opened, title: title, action: action, submitButtonText: submitButtonText, cancelButtonText: cancelButtonText },
        });
      },
    };
  }),
